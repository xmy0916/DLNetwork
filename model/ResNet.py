    ...
    if pretrained:
        ...
        if "num_classes" in kwargs.keys():
            if int(kwargs["num_classes"]) != 1000:
                weight = torch.Tensor(random.rand(int(kwargs["num_classes"]), 2048))
...
def resnet(saveFeature=False, cfg=None):
    layers = {
        'resnet18':['resnet18', BasicBlock, [2, 2, 2, 2]],
        'resnet34':['resnet34', BasicBlock, [3, 4, 6, 3]],
        'resnet50':['resnet50', Bottleneck, [3, 4, 6, 3]],
        'resnet101':['resnet101', Bottleneck, [3, 4, 23, 3]],
        'resnet152':['resnet152', Bottleneck, [3, 8, 36, 3]]
    }
    return _resnet(*layers[cfg['name']], cfg["pretrained"], progress = True,num_classes=cfg["num_class"])
